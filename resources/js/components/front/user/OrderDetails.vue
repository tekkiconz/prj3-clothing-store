        ...
        <div class="modal-dialog modal-custom">
            <div class="modal-content">
                <div class="modal-header text-right">
                    <h3 class="modal-title">Invoice<strong>#{{ order_no }}</strong></h3>
                ...
                <div class="modal-body">
                    <div class="row pb-2 invoice-header">
                        ...
                        <div class="col-md-6 text-right">
                            ...
                            <p><span class="text-muted">Order Placed: </span> {{ order.order_date | dateToString }}</p>
                            <p v-if="order.status == 3"><span class="text-muted">Delivery Date: </span>
                                {{ order.delivery_date | dateToString }}</p>
                        </div>
                    </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-for="value in detials_info" :key="value.id">
                                            <td>
                                                <cld-image :publicId="value.product.product_image" loading="lazy"
                                                           width="50" height="40" alt=".webp not supported in safari"></cld-image>
                                            </td>
                                            <td>{{ value.product.product_name }} <br>
                                                <small>{{ value.product.quantity_unit }}</small></td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="thick-line">{{ currency.symbol }}
                                        ...
                                        <tr>
                                            ...
                                            <td class="no-line">{{ currency.symbol }}
                    ...
                    <div class="row">
                        ...
                        <div class="col-2 mt-4">
                            <a :href="url+'user-order-details-pdf/'+order_no"
                               class="btn btn-primary float-center btn-sm">PDF</a>
...
export default {
    ...
    methods: {
        ...
        userOrderDetails() {
            // this.order_no = id
            this.isLoading = true
            axios.get(base_url + 'user/order/' + this.order_no + '/details')
                .then(response => {
                    // console.log(response.data)
                    this.detials_info = response.data
